...
//#region  Para la busqueda General
...
function buscarHospitales(busqueda, regex) {
  return new Promise((resolve, reject) => {
    HospitalModelo.find({ nombre: regex })
...
function buscarMedicos(busqueda, regex) {
  return new Promise((resolve, reject) => {
    MedicoModelo.find({ nombre: regex })
      .populate('usuario', 'nombre email')
      .populate('hospital')
      .exec((err, medicos) => {
        if (err) {
...
function buscarUsuarios(busqueda, regex) {
  return new Promise((resolve, reject) => {
    UsuarioModelo.find({}, 'nombre email role')
      .or([{ 'nombre': regex }, { 'email': regex }])
      .exec((err, usuarios) => {
        if (err) {
          reject('Error al recuperar Usuarios...!')
        } else {
          resolve(usuarios)
        }
      })
...
//#endregion fin busqueda general
module.exports = app;